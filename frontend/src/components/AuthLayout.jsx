...
export const AuthLayout = ({ children, title, subtitle, isCardFading = false, isZooming = false }) => {
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-[450px] space-y-6 relative z-10">
        ...
        <div className={`flex flex-col items-center gap-3 text-center animate-fade-in-up ${isZooming ? 'zoom-out-animation' : ''} ${isCardFading && !isZooming ? 'opacity-100' : ''}`}>
        ...
        {/* Card Container with Neon Border - Fades first */}
        <div className={`glass-card rounded-2xl shadow-2xl p-8 border-2 border-white/50 animate-scale-in backdrop-blur-xl animate-neon-border transition-opacity duration-400 ${isCardFading ? 'opacity-0' : 'opacity-100'}`}>
